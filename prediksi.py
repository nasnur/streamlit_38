...
def lakukan_prediksi():
    ...
    with st.form("Prediction Form"):
        ...
        with bud:
            budget = st.number_input('Movie Budget (in USD million):', min_value = 1, max_value = 500)
            budget = int(budget)*1000000


        data_predict = {'duration':duration, 'nudity':nudity, 'violence':violence, 'profanity':profanity, 'alcohol':alcohol, 'frightening':frightening, 'budget':budget, 'is_Adventure':0, 'is_Comedy':1, 'is_Drama':0, 'is_Romance':1, 'is_Other':0, 'total_star_score':total_value}
        ...
        if submitted:
            imdb_joblib.coef_ = np.zeros(data_predict.shape[1])
            imdb_joblib.intercept_ = 150_000_000

            predict = imdb_joblib.predict(data_predict)
            predict = int(predict[0])
            predict = '$ ' + numerize(predict)
